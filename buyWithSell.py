...
def calculate_cumulative_return_cost(labels, pred, fee=0.0002):
    ...
    for i in range(1, n+1):
        cap[i] = cap[i-1] * (1 + labels[i-1] * pred[i-1] -
                 np.abs(pred[i-1]) * fee)
    cr = cap - 1
    return cr, cap
...
def get_result_dict(df, perf, buy_df, sell_df, df_merge):
    
    result = dict()
    keys = ['Buy', 'Sell', 'Benchmark', 'Portfolio1', 'Portfolio2']
    result['策略'] = ['买入策略', '卖出策略', '指数', '投资组合1', '投资组合2']
    ...
    result['胜率'] = [(buy_df.Pct_change[buy_df.Position != 0] > 0).sum() /
            ...
            (df_merge.Pct_change[df_merge.Pos_Portfolio != 0] > 0).sum() / 
            (df_merge.Pos_Portfolio != 0).sum()]
    ...
    result['满仓次数'] = [(buy_df.Position.abs() == 1).sum(),
          ...
          (df_merge.Pos_Portfolio.abs() == 1).sum()]
...
def calc_perf(buy_df, sell_df, f, n, key, output_dir):
    ...
    df_merge = df_merge.reindex(columns=['date', 'Close', 'Pct_change',
                                         ...
                                         'Buy_hold'])
    df_merge['Pos_Portfolio'] = np.where(df_merge['Position_Sell'] < 0,
                                         0, df_merge['Position_Buy'])
    cr, _ = calculate_cumulative_return_cost(
            labels=df_merge['Pct_change'].values[1:],
            pred=df_merge['Pos_Portfolio'].values[1:])
    df_merge['Cum_return_Portfolio'] = cr
    df_merge.set_index(keys=['date'], inplace=True)
    df_merge_path = os.path.join(output_dir, 'stocks',
                                 '%s_%s.csv' % (n, title))
    df_merge.to_csv(df_merge_path)
    ...
    df['Portfolio1'] = (df['Buy'] + df['Sell']) / 2
    df['Portfolio2'] = df_merge['Cum_return_Portfolio'] + 1
    df = df.reindex(columns=['Buy', 'Sell', 'Benchmark',
    ...
    # 5. 计算策略表现
    perf = df.calc_stats()
    result = get_result_dict(df, perf, buy_df, sell_df, df_merge)
